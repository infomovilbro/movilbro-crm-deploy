...
(async () => {
  ...
  // Escribir con pkill para matar server anterior
  var cmd = 'git fetch --all && git reset --hard origin/main && pkill -9 -f node && sleep 1 && node server.js';
  for (var ch of cmd) {
  ...
  try {
    ...
    console.log('Login OK?', p2.url().includes('tienda') ? 'SI' : 'NO');
