...
function initDatabase() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS isp_nodos (id INTEGER PRIMARY KEY AUTOINCREMENT, nombre TEXT NOT NULL, direccion TEXT, tipo TEXT DEFAULT 'nodo', latitud REAL, longitud REAL, estado TEXT DEFAULT 'activo', notas TEXT, serie TEXT DEFAULT 'F', numero_factura INTEGER DEFAULT 0, created_at DATETIME DEFAULT CURRENT_TIMESTAMP);
  ...
  try { db.prepare("ALTER TABLE clients ADD COLUMN lineas_pago TEXT DEFAULT '{}'").run(); } catch(e) {}
  ...
  try { db.prepare("ALTER TABLE pending_messages ADD COLUMN altas_score REAL DEFAULT 0").run(); } catch(e) {}
  try { db.prepare("ALTER TABLE pending_messages ADD COLUMN contact_name TEXT").run(); } catch(e) {}
  try { db.prepare("ALTER TABLE pending_messages ADD COLUMN contact_phone TEXT").run(); } catch(e) {}
  try { db.prepare("ALTER TABLE isp_cdrs ADD COLUMN factura_id INTEGER").run(); } catch(e) {}
  try { db.exec("CREATE TABLE IF NOT EXISTS isp_series (id INTEGER PRIMARY KEY AUTOINCREMENT, serie TEXT UNIQUE NOT NULL, nombre TEXT, ultimo_numero INTEGER DEFAULT 0, ultimo_ejercicio TEXT DEFAULT '', created_at DATETIME DEFAULT CURRENT_TIMESTAMP)").run(); } catch(e) {}
  try { db.prepare("INSERT OR IGNORE INTO isp_series (serie, nombre, ultimo_numero, ultimo_ejercicio) VALUES ('F', 'Facturas', 0, '')").run(); } catch(e) {}
