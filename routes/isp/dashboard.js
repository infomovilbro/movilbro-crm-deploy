...
router.get('/', async (req, res) => {
  try {
    ...
    try {
      morosos = db.prepare(`
        SELECT fiscal_id, cliente_nombre, SUM(importe_total) as total_adeudado, COUNT(*) as facturas_vencidas, MIN(periodo) as periodo_mas_antiguo
        FROM isp_facturas WHERE estado='pendiente' AND fecha_vencimiento < date('now')
        GROUP BY fiscal_id ORDER BY total_adeudado DESC
      `).all();
