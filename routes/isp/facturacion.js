  ...
  try {
    ...
    if (cdrs.length === 0) {
      try {
        ...
        if (Array.isArray(apiCdrs)) cdrs = apiCdrs;
        else if (apiCdrs && apiCdrs.data) cdrs = apiCdrs.data;
        else if (apiCdrs && apiCdrs.cdrs) cdrs = apiCdrs.cdrs;
        // Filtrar por periodo
        if (factura.periodo) {
          cdrs = cdrs.filter(function(c) {
            var d = c.fecha || c.date || '';
            return d.substring(0, 7) === factura.periodo;
          });
        }
