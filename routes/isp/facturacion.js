  ...
  try {
    ...
    if (cdrsDetalle.length === 0 && factura.fiscal_id) {
      try {
        ...
        // Si obtuvimos CDRs de API, extraer llamadas para el detalle
        if (cdrsDetalle.length > 0) {
          cdrsDetalle.forEach(function(c) {
            if (c.tipo === 'voice' || c.tipo === 'llamada' || c.tipo === 'calls' || c.tipo === 'call') {
              llamadas.push({
                fecha: c.fecha || c.date || '',
                hora: c.hora || c.time || c.startTime || '',
                destino: c.destino || c.destination || c.number || c.phone || c.calledNumber || '',
                grupo: c.grupo || c.group || c.nombre_grupo || c.name_group || '',
                duracion: c.duracion || c.duration || c.seconds || c.unidades || '',
                importe: parseFloat(c.importe || c.import || c.price || c.cost || 0)
              });
            }
          });
        }
...
router.get('/facturas/:id', async (req, res) => {
  try {
    ...
    if (cdrsDetalle.length === 0 && factura.fiscal_id) {
      try {
        ...
        if (lineasUnicas.length === 0) {
          try { var lineasDb = db.prepare("SELECT value FROM settings WHERE key='all_lines'").get(); if (lineasDb) { lineasUnicas = JSON.parse(lineasDb.value); } } catch(e2) {}
        }
        ...
        apiCdrsResults.forEach(function(resp) {
          ...
          if (Array.isArray(items)) {
            items.forEach(function(item) {
              var cdrDate = item.fecha || item.date || item.createdAt || item.created_at || '';
              var cdrPeriodo = cdrDate ? cdrDate.substring(0, 7) : factura.periodo;
              var linea = item.linea || item.lineNumber || item.line || item.destino || item.destination || item.number || item.phone || item.calledNumber || lineasUnicas[0] || '';
              cdrsDetalle.push({
                fecha: cdrDate,
                concepto: item.concepto || item.concept || item.description || 'Consumo',
                linea: linea,
                tipo: (item.tipo || item.type || item.callType || 'exceso').toLowerCase(),
                unidades: parseFloat(item.unidades || item.units || item.duracion || item.duration || item.seconds || item.minutos || item.mb || 0),
                importe: parseFloat(item.importe || item.import || item.price || item.cost || 0),
                destino: item.destino || item.destination || item.number || item.phone || item.calledNumber || ''
              });
            });
          }
        });
    ...
    // Calcular total CDRs para mostrar arriba
    var totalCdrs = cdrsDetalle.reduce(function(s, c) { return s + parseFloat(c.importe || 0); }, 0);
    // Obtener CDRs de uso diario (agrupados por día) desde API o DB
    var usoDiario = [];
    if (cdrsDetalle.length > 0) {
      var daily = {};
      cdrsDetalle.forEach(function(c) {
        var dia = (c.fecha || c.date || '').substring(0, 10);
        if (!dia) return;
        if (!daily[dia]) daily[dia] = { fecha: dia, data: 0, voice: 0, sms: 0, total: 0 };
        var imp = parseFloat(c.importe || 0);
        var tipo = (c.tipo || 'data').toLowerCase();
        if (tipo === 'voice') daily[dia].voice += imp;
        else if (tipo === 'sms') daily[dia].sms += imp;
        else daily[dia].data += imp;
        daily[dia].total += imp;
      });
      usoDiario = Object.keys(daily).sort().map(function(k) { return daily[k]; });
    }

    res.render('isp/facturacion/detalle', { title: 'Factura #' + factura.id, factura, lineas, cdrsDetalle, llamadas, totalCdrs, usoDiario });
  } catch(e) { res.status(500).send('Error: ' + e.message); }
});
...
// Descargar factura como HTML imprimible (fallback PDF sin playwright)
router.get('/facturas/:id/pdf', async (req, res) => {
  try {
    ...
    for(var gk in cdrG){var g=cdrG[gk];lineas.push({concepto:g.concepto,tipo:'cdr',importe:Math.round(g.total*100)/100,linea:g.linea});}
    var cdrsDetalle = []; try { cdrsDetalle = db.prepare('SELECT * FROM isp_cdrs WHERE factura_id=?').all(req.params.id); } catch(e) {}
    var llamadas = []; try { llamadas = db.prepare('SELECT * FROM isp_llamadas WHERE factura_id=? ORDER BY fecha, hora').all(req.params.id); } catch(e) {}
    var ejs = require('ejs'), fs = require('fs'), path = require('path');
    var tpl = fs.readFileSync(path.join(__dirname, '..', '..', 'views', 'isp', 'facturacion', 'invoice-html.ejs'), 'utf8');
    var html = ejs.render(tpl, { factura, lineas, cdrsDetalle, llamadas, history: [], layout: false });
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.setHeader('Content-Disposition', 'inline; filename="factura-' + factura.id + '.html"');
    res.send(html);
  } catch(e) { res.status(500).send('Error: ' + e.message); }
});

// Cambiar método de pago de una factura
router.post('/facturas/:id/metodo-pago', (req, res) => {
  try {
    db.prepare('UPDATE isp_facturas SET metodo_pago=? WHERE id=?').run(req.body.metodo_pago, req.params.id);
    res.json({ ok: true });
  } catch(e) { res.status(500).json({ ok: false, error: e.message }); }
});
...
// Obtener CDRs/llamadas de una linea en un periodo
router.get('/facturas/:id/linea/:linea', async (req, res) => {
  try {
    var factura = db.prepare('SELECT * FROM isp_facturas WHERE id=?').get(req.params.id);
    if (!factura) return res.status(404).json({ error: 'No encontrada' });
    
    var cdrs = [];
    try { cdrs = db.prepare("SELECT * FROM isp_cdrs WHERE factura_id=? AND linea=? ORDER BY created_at").all(req.params.id, req.params.linea); } catch(e) {}
    var llamadas = [];
    try { llamadas = db.prepare("SELECT * FROM isp_llamadas WHERE factura_id=? ORDER BY fecha, hora").all(req.params.id); } catch(e) {}
    ...
    if (cdrs.length === 0) {
      try {
        var api = LikesAPI.getApiInstance();
        var apiCdrs = await api.getLineCDRs(req.params.linea);
        var rawCdrs = [];
        if (Array.isArray(apiCdrs)) rawCdrs = apiCdrs;
        else if (apiCdrs && apiCdrs.data) rawCdrs = apiCdrs.data;
        else if (apiCdrs && apiCdrs.cdrs) rawCdrs = apiCdrs.cdrs;
        else if (apiCdrs && apiCdrs.records) rawCdrs = apiCdrs.records;
        else if (apiCdrs && apiCdrs.items) rawCdrs = apiCdrs.items;
        if (Array.isArray(rawCdrs)) {
          cdrs = rawCdrs.map(function(c) {
            return {
              fecha: c.fecha || c.date || c.createdAt || c.created_at || '',
              hora: c.hora || c.time || c.startTime || '',
              destino: c.destino || c.destination || c.number || c.phone || c.calledNumber || '',
              grupo: c.grupo || c.group || c.nombre_grupo || c.name_group || '',
              duracion: c.duracion || c.duration || c.seconds || c.unidades || '',
              tipo: (c.tipo || c.type || c.callType || 'voice').toLowerCase(),
              importe: parseFloat(c.importe || c.import || c.price || c.cost || 0)
            };
          });
        }
      } catch(e) { console.error('API line CDR error:', e.message); }
    }
    
    res.json({ ok: true, cdrs: cdrs, llamadas: llamadas });
  } catch(e) { res.status(500).json({ ok: false, error: e.message }); }
});
