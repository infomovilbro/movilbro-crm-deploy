...
router.get('/', async (req, res) => {
  try {
    ...
    // Also fetch clients from API Likes to show API-only clients
    var apiClientes = [];
    try {
      ...
      if (Array.isArray(customers)) {
        apiClientes = customers.map(function(c) {
          ...
          var nombre = c.name + ' ' + (c.firstSurname || c.lastName || '');
          return { id: fiscalId, nombre: nombre.trim(), fiscal_id: fiscalId };
        }).filter(function(c) { return c.fiscal_id; });
      }
    } catch(e) { console.error('[CDRs] API customers fetch:', e.message); }
    
    res.render('isp/cdrs/index', { title: 'CDRs y Excedentes', groups: grupos, total: total, clientes: clientes, apiClientes: apiClientes, yearsMap: yearsMap, selectedPeriodo: selectedPeriodo, MES_NOMBRES: ['','Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'] });
...
// Obtener CDRs de un cliente especifico desde API Likes
router.get('/api-cliente/:fiscalId', async (req, res) => {
  try {
    ...
    // Obtener suscripciones/lineas del cliente
    try {
      ...
      subsItems.forEach(function(s) {
        ...
        prods.forEach(function(p) {
          var num = p.fixedNumber || p.lineNumber || '';
          if (num) lineas.push(num);
        });
      });
    } catch(e) {}
    ...
    // Obtener CDRs por cada linea
    for (var i = 0; i < lineas.length; i++) {
      try {
        var raw = await api.getLineCDRs(lineas[i]);
    ...
    // Ordenar por fecha
    cdrs.sort(function(a, b) { return (a.fecha || '').localeCompare(b.fecha || ''); });
    
    res.json({ ok: true, fiscalId: fiscalId, lineas: lineas, cdrs: cdrs, total: cdrs.reduce(function(s, c) { return s + c.importe; }, 0) });
  } catch(e) { res.status(500).json({ ok: false, error: e.message }); }
});
