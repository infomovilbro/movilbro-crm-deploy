  ...
  try {
    ...
    try {
      ...
      if (driveHelper.isAvailable()) {
        ...
        var kycFolder = await driveHelper.ensureFolder(driveRootId, folderName);
        if (kycFolder) {
          driveFolderId = kycFolder;
          var google = require('googleapis').google;
          var drive2 = google.drive({ version: 'v3', auth: require('../helpers/drive').getAuth() });
          var fs2 = require('fs');
          var fileResult = await drive2.files.create({
            requestBody: { name: (req.file.originalname || 'doc_' + Date.now() + '.jpg'), parents: [kycFolder] },
            media: { mimeType: req.file.mimetype || 'image/jpeg', body: fs2.createReadStream(req.file.path) },
  ...
  try {
    ...
    try {
      ...
      if (driveHelper.isAvailable()) {
        ...
        var driveRootId = process.env.DRIVE_ROOT_FOLDER_ID || '1JrStvTy-l0msOmfwT1S0Jupg6Ru6Zemx';
        var contratoFolder = await driveHelper.ensureFolder(driveRootId, folderName);
        if (contratoFolder) {
          try {
            var google = require('googleapis').google;
            var drive2 = google.drive({ version: 'v3', auth: require('../helpers/drive').getAuth() });
            await drive2.files.create({
              requestBody: { name: 'contrato_firmado_' + orden.token + '.png', parents: [contratoFolder] },
              media: { mimeType: 'image/png', body: require('stream').Readable.from(sigBuf) },
              fields: 'id'
            });
          } catch(e2) { console.error('[KYC] Drive contrato upload error:', e2.message); }
        }
      }
