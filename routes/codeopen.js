...
router.post('/approve/:id', async (req, res) => {
  try {
    ...
    // Helper para crear opts limpios por llamada
    function makeOpts(extra) {
      var o = {};
      if (mode === 'with_forward' && row.quoted_data) o.quotedData = row.quoted_data;
    ...
    if (row.source === 'whatsapp' || row.category === 'whatsapp') {
      try {
        ...
        // Con reenvío pero sin quoted_data → texto con cita
        if (mode === 'with_forward' && !row.quoted_data) {
          var textWithQuote = '📩 Mensaje original:\n"' + (row.body || '') + '"\n\n---\n\n' + responseText;
          var result = await wa.sendMessage(row.from_address, textWithQuote, makeOpts());
        ...
        if (!sent && asAudio && responseText) {
          try {
            ...
            if (audioBuf) {
              var result = await wa.sendMessage(row.from_address, { audioBuffer: audioBuf, mimeType: 'audio/mp3', text: responseText }, makeOpts({ asAudio: true }));
          ...
          // Si audio falló, reintentar como texto
          if (!sent) {
            var textResult = await wa.sendMessage(row.from_address, responseText, makeOpts());
            if (textResult && textResult.ok) { sent = true; sendInfo += ' (enviado como texto)'; }
          }
        }

        // Enviar como texto (default solo si NO es audio ni documento)
        if (!sent) {
          var result = await wa.sendMessage(row.from_address, responseText, makeOpts());
...
// ---- findClientByPhone: busca fiscal ID a partir de número de teléfono ----
async function findClientByPhone(phone) {
  try {
    var cleanPhone = String(phone).replace(/[^0-9]/g, '');
    if (cleanPhone.length < 6) return null;
    var client = db.prepare("SELECT dni_nif, likes_customer_id FROM clients WHERE telefono LIKE ? OR telefono2 LIKE ? LIMIT 1").get('%' + cleanPhone + '%', '%' + cleanPhone + '%');
    if (client && client.dni_nif) return client.dni_nif;
    if (client && client.likes_customer_id) return client.likes_customer_id;
    try {
      ...
      if (Array.isArray(customers)) {
        for (var c of customers) {
          ...
          if (cPhone && (cPhone.includes(cleanPhone) || cleanPhone.includes(cPhone) || cleanPhone.slice(-9) === cPhone.slice(-9))) {
            var fid = c.fiscalId || c.fiscal_id || c.dni || '';
            if (fid) return fid;
          }
        }
      }
...
// BAILEYS QR: Rutas públicas en server.js (NO duplicar aquí)
