...
router.get('/', requireAuth, async (req, res) => {
  try {
    ...
    if (!Array.isArray(products) || products.length === 0) {
      products = db.prepare("SELECT id, nombre as productName, precio as price, tipo as family FROM products ORDER BY nombre").all();
    }
...
router.post('/crear-cliente-avanzado', requireAuth, async (req, res) => {
  try {
    const api = getApi();
    var { nombre, apellidos, email, telefono, dni, direccion, ciudad, codigo_postal, customerType, metodo_pago, iban, productos, tipo_contratacion, lineas, cobertura_id } = req.body;
    ...
    const datosProducto = JSON.stringify({ productos: productos, producto_id, tipo_contratacion, lineas, cobertura_id });
...
// GET /altas/kyc-docs/:id - get KYC documents for an order
router.get('/kyc-docs/:id', requireAuth, async (req, res) => {
  try {
    const docs = db.prepare('SELECT * FROM altas_kyc_docs WHERE orden_id = ? ORDER BY created_at').all(req.params.id);
...
// POST /altas/aprobar-orden - approve order after KYC review
router.post('/aprobar-orden', requireAuth, async (req, res) => {
  try {
    const { orden_id } = req.body;
    const orden = db.prepare('SELECT * FROM altas_ordenes WHERE id = ?').get(orden_id);
    if (!orden) return res.status(404).json({ ok: false, error: 'Orden no encontrada' });
    db.prepare("UPDATE altas_ordenes SET estado = 'pendiente_aprobacion', updated_at = CURRENT_TIMESTAMP WHERE id = ?").run(orden_id);
    res.json({ ok: true, message: 'Orden marcada como pendiente de aprobacion. Usa Completar para enviar a API.' });
  } catch (error) {
    res.status(500).json({ ok: false, error: error.message });
  }
});

// POST /altas/rechazar-orden - reject order with reason
router.post('/rechazar-orden', requireAuth, async (req, res) => {
  try {
    const { orden_id, motivo } = req.body;
    db.prepare("UPDATE altas_ordenes SET estado = 'rechazada', notas = COALESCE(notas,'') || ? || '\n', updated_at = CURRENT_TIMESTAMP WHERE id = ?").run('RECHAZADO: ' + (motivo || 'Sin motivo') + ' (' + new Date().toLocaleString('es-ES') + ')', orden_id);
...
// Sync env vars to DB settings at startup (handled by services/email.js)

// Verificar viabilidad de un cliente antes del alta
router.post('/check-viabilidad', requireAuth, async (req, res) => {
  try {
    var { dni, telefono } = req.body;
    var result = { viable: true, clienteExistente: false, advertencias: [], scoring: null, aeat: null };
    
    // Buscar en clientes locales
    if (dni) {
      var local = db.prepare('SELECT id, nombre, telefono FROM clients WHERE dni_nif=?').get(dni);
      if (local) {
        result.clienteExistente = true;
        result.advertencias.push('Cliente ya registrado: ' + (local.nombre || '') + ' (Telf: ' + (local.telefono || 'sin telefono') + ')');
      }
    }
    if (telefono) {
      var localTel = db.prepare('SELECT id, nombre, dni_nif FROM clients WHERE telefono=? OR telefono2=?').get(telefono, telefono);
      if (localTel && (!dni || localTel.dni_nif !== dni)) {
        result.clienteExistente = true;
        result.advertencias.push('Teléfono ya registrado: ' + (localTel.nombre || '') + ' (DNI: ' + (localTel.dni_nif || '') + ')');
      }
    }
    ...
    try {
      ...
      if (Array.isArray(customers)) {
        for (var c of customers) {
          var fiscalMatch = dni && (c.fiscalId === dni || c.fiscal_id === dni || c.dni === dni);
          var phoneMatch = telefono && ((c.phone || '').replace(/[^0-9]/g, '').includes(telefono.replace(/[^0-9]/g, '')) || telefono.replace(/[^0-9]/g, '').includes((c.phone || '').replace(/[^0-9]/g, '')));
          if (fiscalMatch || phoneMatch) {
            result.clienteExistente = true;
            result.scoring = c.scoring || c.score || c.rating || null;
            result.aeat = c.aeatStatus || c.aeat_status || null;
            if (!result.advertencias.some(function(a) { return a.includes('API'); })) {
              result.advertencias.push('Cliente encontrado en API Likes: ' + (c.name || '') + (result.scoring ? ' (Scoring: ' + result.scoring + ')' : ''));
            }
            break;
          }
        }
