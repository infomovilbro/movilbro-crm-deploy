...
// POST — Reportar error: guarda en BD + archivo publico para que opencode lo detecte
router.post('/ai-assist/report', async (req, res) => {
  try {
    ...
    // Guardar en archivo publico para que opencode lo lea
    var pubPath = path.join(__dirname, '..', 'public', 'ai-assist-pending.json');
    var data = JSON.stringify({ id: r.lastInsertRowid, text: text, url: url, selector: selector, element_text: element_text, created_at: new Date().toISOString() });
    fs.writeFileSync(pubPath, data);
...
// POST — opencode responde con la solucion (protegido con clave secreta)
router.post('/ai-assist/respond', async (req, res) => {
  try {
    ...
    // Guardar respuesta en archivo publico para el frontend
    var pubPath = path.join(__dirname, '..', 'public', 'ai-assist-response.json');
    fs.writeFileSync(pubPath, JSON.stringify({ response: responseText, tts: true, fix_data: { solution: solution, url: '', selector: '' } }));
    
    // Si hay fix_id, actualizar la nota
    if (fixId) {
      try { db.prepare("UPDATE fix_notes SET status='fixed', fixed_at=datetime('now') WHERE id=?").run(fixId); } catch(e) {}
    }
...
// POST — Limpiar respuesta leida
router.post('/ai-assist-clear', async (req, res) => {
  try {
    var p = path.join(__dirname, '..', 'public', 'ai-assist-response.json');
