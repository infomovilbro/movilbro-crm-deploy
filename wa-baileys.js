...
async function initBaileys(pairingPhone) {
  try {
    ...
    // Si se proporciona numero de telefono, solicitar codigo de emparejamiento
    if (pairingPhone) {
      socketConfig.phoneNumber = pairingPhone;
      console.log('[Baileys] Solicitar codigo de emparejamiento para', pairingPhone);
    }
...
async function requestPairingCode(phoneNumber) {
  try {
    ...
    console.log('[Baileys] Pairing code requested for', phoneNumber, ':', code);
    return code;
  } catch(e) {
    console.error('[Baileys] Pairing error:', e.message);
    return null;
  }
}

module.exports = { initBaileys, getQRDataURL, getStatus, sendMessage: sendBaileysMessage, transcribeAudioMessage, getChats, getChatMessages, getProfilePicture, end, requestPairingCode };
