...
## Sesi�n 2026-08-04 � Instalaci�n de Herramientas + Nuevo Flujo de Trabajo

### Herramientas instaladas (config global opencode, NO en el repo)
| Herramienta | Qu� hace | Estado |
|---|---|---|
| **Superpowers** (obra/superpowers) | Metodolog�a: brainstorming ? plan ? ejecuci�n ? revisi�n. Skills en ~/.config/opencode/node_modules/superpowers | ? |
...
### NUEVO FLUJO DE TRABAJO OBLIGATORIO (m�todo programador)
El admin me muestra c�mo est� algo. Yo sigo EXACTAMENTE este flujo:

1. **VERIFICAR primero** � Con CDP (navegador depurado puerto 9222), MCP (consulta SQLite directa), lectura de c�digo, respuestas reales de la API. NUNCA inventar.
2. **SIMULAR antes de tocar** � Creo un HTML/archivo de simulaci�n LOCAL (crm_sim.html, etc.) que el admin ve en su navegador. Le ense�o C�MO QUEDAR�A arreglado SIN tocar el servidor.
3. **APROBACI�N** � El admin ve la simulaci�n. Solo si le gusta, paso al paso 4.
4. **IMPLEMENTAR** � Solo entonces toco el c�digo real del servidor (routes/, views/, etc.).
5. **VERIFICAR post-cambio** � Comprobar que lo que ya funcionaba sigue funcionando.
6. **Commit + push + deploy** � Al final de la sesi�n, UN solo commit + push a main + Manual Deploy.
7. **Verificar en navegador real** � Confirmar en la web de Render antes de informar.

### Reglas cr�ticas del nuevo flujo
- **NUNCA implementar directo en el servidor sin simulaci�n previa aprobada.** Si el admin solo pidi� ver algo, mostrar simulaci�n, no tocar c�digo.
- **NUNCA usar 
...
- **El config opencode.json del proyecto es LOCAL** � est� en .gitignore, no se sube.

### ?? REGLA CR�TICA: Verificar VISIBILIDAD REAL, no solo existencia en DOM
...
- **Comprobar la estructura:** los .tab-pane deben ser HERMANOS directos de .tab-content, nunca anidados entre s�.
- **Hacer click REAL (playwright click + snapshot), no solo el.click() por JS.**
- Si el usuario dice "lo veo vac�o" y t� "ves" contenido con evaluate ? el problema es de estructura/visibilidad, no de datos.
- Un </div> que falta en un pane anida todos los siguientes dentro de �l ? crash visual masivo.

### ?? REGLA CR�TICA: Verificar que cada bot�n llega a la API REAL (no inventar)
- **UN BOT�N NO FUNCIONA SOLO POR TENER HTML/CSS.** Debe hacer un fetch a un endpoint del backend que llame a la API de Likes Telecom con el FORMATO EXACTO que acepta.
- **Antes de dar un bot�n por bueno, probar el endpoint contra la API real** (https://api.likestelecom.com) con el token, y verificar el formato del body.
- Formato real verificado (agosto 2026):
  - L�mite cr�dito: PUT /line/credit-limit ? {lineNumber, creditLimit, toBeBlocked, autoUnlocking} (NO limit)
...
- **El modal de l�nea (abrirLineModal) ya carga en PARALELO** las llamadas a la API para que cargue rapido.
- Si un endpoint da 403 FORBIDDEN con token directo pero la CRM de Likes lo usa, puede requerir el token del navegador (m�s permisos).
